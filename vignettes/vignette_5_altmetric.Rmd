...
```{r impact_almetric_plot1a, fig.align=, fig.height=5.76, fig.width=9.60, echo=TRUE, include=FALSE}
almetric$temporal %>% 
  ggplot() +
  aes(x = alm_time, y = alm_score, group = pmid, colour = pmid) +
  geom_line() + geom_point() + theme_bw()
```
```{r impact_almetric_plot1b, fig.align=, fig.height=5.76, fig.width=9.60, }
(almetric$temporal %>% 
  ggplot() +
  aes(x = alm_time, y = alm_score, group = pmid, colour = pmid) +
  geom_line() + geom_point() + theme_bw()) %>%
  ggsave(filename="~/impactr/vignettes/plot/alm_plot1.png",height=5.76, width = 9.60, units ="in")
```
...
```{r impact_alm_rank, warning=FALSE, echo=FALSE, message=FALSE, fig.height = 5.76, fig.width = 9.60, fig.align = "center"}
almetric$rank %>%
...
```{r impact_alm_source, warning=FALSE, echo=FALSE, message=FALSE, fig.height = 5.76, fig.width = 9.60, fig.align = "center"}
almetric$source %>%
  head(12) %>%
  knitr::kable(format="html") %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = F)
