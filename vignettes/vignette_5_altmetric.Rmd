...
## **Output**
There are 4 outputs from `impact_altmetric()` as nested dataframes: `$df_output`, `$temporal`, `$rank`, and `$source`.
...
col_set1 <- which(colnames(altmetric$df_output) %in% c("title"))
col_set2 <- which(colnames(altmetric$df_output) %in% c("author_list"))
col_set3 <- which(colnames(altmetric$df_output) %in% c("doi", "date_update", "date_pub", "date_added"))
